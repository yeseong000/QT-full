"""
떠오르는 질문(follow_up) GPT 시스템 프롬프트 합본 빌드 스크립트.

사용법:
    python build.py

동작:
    parts/*.md 파일들을 알파벳(번호) 순으로 모아 _final_system.md를 생성합니다.
    이 합본 파일이 실제 GPT API 호출 시 system 프롬프트로 사용됩니다.
    (※ 코드 연결 전까지 라이브는 기존 system.md를 그대로 사용합니다.)
"""
