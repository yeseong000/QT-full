...
def format_commentary_for_prompt(verse_commentaries: dict, book_ko: str | None = None,
    ...
    if not verse_commentaries:
        return None
    lines = [
        "[공개 주석 — '이 구절이 왜 이런 뜻인지' 설명한 실제 자료다(영문). "
        "배경·신학을 밝힐 근거로 삼되, 주석마다 견해가 갈리면 '주의점'에 견해차를 명시하라. 없는 내용은 지어내지 마라. "
        "★ 시대를 보고 골라라. Gill(1746)·Clarke(1810)·Keil-Delitzsch(1866)·JFB(1871)·Matthew Henry(1710)는 "
        "본문 흐름·히브리어 어휘·신학적 의미에 강하다. 반면 연대·지리·고대 관습·과학처럼 성경 밖 증거에 기대는 "
        "항목은 20세기 발굴 이후 바뀐 게 많아 이 옛 주석들이 틀렸을 수 있다. "
        "cat이 '문화관습'·'지리'·'인물배경'인 항목은 Tyndale(2022)을 우선 근거로 삼고, Tyndale에 해당 내용이 "
        "없으면 옛 주석을 쓰되 confidence를 medium으로 낮춰라. "
