...
# 2026-08-04 개편 — '연결 질문'을 없애고 '관주'·'전날'로 가른다.
# 근거: 7~8월 '연결' 카테고리로 나간 질문 17개 중 17개가 근거(anchor) 없이 나갔다.
# "다른 성경 사례가 있나요?"라고 묻는데 어떤 구절인지 아무도 대조하지 않아,
# "'사십'을 '사 년'으로 번역한 예는 성경에서 종종 발견돼요" 같은 거짓 일반화가 통과했다.
# 관주·전날은 코드가 원자료와 대조하는 자리라, 카테고리를 이 둘로 바꾸면
# '근거 없는 연결 질문'이 애초에 성립하지 않는다. 각도(무엇을 묻나)와 근거(어디서 확인하나)가
# 1:1로 맞아떨어진다.
CATEGORIES = (
    "주석형/본문관찰", "지명 정보", "어원·유래", "인물 배경", "문화·관습",
    "신학/해석 견해", "본문 디테일", "번역·사본", "관주", "전날", "랜덤",
)
# KB key_details의 cat 값(어원/지리/문화관습·문화/신학핵심)과 STEP2 카테고리 매핑.
# "신학/해석 견해"는 항상 존재하는 챕터 top-level 필드(신학_핵심·주의점)로도 채워짐.
KB_CAT_PREFIX = {"지명 정보": "지리", "어원·유래": "어원", "문화·관습": "문화",
                 "신학/해석 견해": "신학", "번역·사본": "사본"}
# 그날 본문 텍스트만으로 항상 채울 수 있는 카테고리 — KB 유무와 무관하게 매일 요청.
# 관주·전날은 그날 재료(관주 목록·최근 본문)가 있어야 성립하므로 ALWAYS에 넣지 않는다.
ALWAYS_FILLABLE = ("주석형/본문관찰", "인물 배경", "본문 디테일", "랜덤")
# 메인 후보로 우선 배치할 카테고리(주제를 여는 느낌에 가까운 순서).
MAIN_PREFERRED = ("관주", "전날", "주석형/본문관찰", "인물 배경", "본문 디테일", "신학/해석 견해")
