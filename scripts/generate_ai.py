...
3. book_overview: 이 책({qt_data['book_name']}) 전체에 대한 소개. 오늘 본문이나 특정 구절은 절대 언급하지 말 것.
   ...
   - "author" (저자): 누가 썼다고 전해지는지. 한두 어절로 짧게.
     ✅ "사도바울" / "전통적으로 사무엘로 전해짐" / "알려지지 않음 (여러 견해)"
     ❌ "사도바울입니다" 같은 격식체 종결.
     저자 논쟁이 있는 책(히브리서·베드로후서·모세오경·이사야·욥기 등)은
     "알려지지 않음", "여러 견해" 같은 완충 표현 필수. 단정 금지.

   - "date" (시기): **객체** {{ "covered": ..., "written": ... }}
     · "written": 기록된 시기 (필수). 명사형 종결.
       예: "AD 60~63년경 추정" / "BC 6세기경 추정"
     · "covered": 책이 다루는 시대 (선택). 기록 시기와 다른 경우에만 채움.
       예: "BC 11세기" / "BC 11세기 사사기"
       기록 시기와 거의 같으면 null로 둘 것 (신약 서신 등).
     예시:
       - 에베소서: {{ "covered": null, "written": "AD 60~63년경 추정" }}
       - 사무엘상: {{ "covered": "BC 11세기", "written": "BC 6세기경 추정" }}
       - 룻기:    {{ "covered": "BC 11세기 사사기", "written": "BC 10~5세기 추정" }}

   - "place" (기록 장소와 대상): 어디서 누구를 위해 쓰였는지 한 문장.
     ✅ "로마의 감옥에서 썼으며 에베소에 있는 그리스도인을 위해 씀."
     ✅ "이스라엘 백성을 위해 기록되었으며, 정확한 장소는 알려지지 않음."
     ❌ "~알려져 있지 않습니다." 같은 격식체 종결.
     `~씀.` `~기록됨.` `~위해 쓰임.` 같은 간결한 명사형/축약 종결 우선.

   - "core" (핵심 내용): 책 전체 줄거리·핵심 메시지. 1~2문장.
     마지막 문장은 **체언으로 마무리**(`~이야기`, `~책`, `~서신`). 특정 구절 인용 금지.
     ✅ "...순종한다는 것이 무엇인지 보여주는 책."
     ❌ "...보여줍니다."

   전체 톤: 한국어, 신학용어는 풀어 설명, 격식체(~입니다/~합니다) X.
   (다른 카드는 ~이에요체이지만, 이 카드만은 사전 항목처럼 명사형 짧은 마무리가 더 깔끔함.)
   교단 편향·논쟁적 해석 금지.
...
{{
  ...
  "book_overview": {{
    ...
    "date": {{
      "covered": "다룬 시대 (기록 시기와 다를 때만, 같으면 null)",
      "written": "기록 시기 한 줄 (필수)"
    }},
    "place": "어디서 누구를 위해 쓰였는지 (한 문장, 명사형 종결)",
    "core": "책 전체 줄거리·메시지 (1~2문장, 체언 종결)"
  }},
...
def _mock_ruth_1(qt_data: dict) -> dict:
    ...
    return {
        ...
        "book_overview": {
            ...
            "date": {
                "covered": "BC 11세기 사사기",
                "written": "BC 10~5세기 추정",
            },
            "place": "이스라엘 백성을 위해 기록되었으며, 정확한 장소는 알려지지 않음.",
            "core": "모압 여인 룻이 시어머니를 따라 베들레헴에 와 보아스와 가정을 이루며, 평범한 삶 속에서도 일하시는 하나님의 손길을 보여주는 이야기.",
        },
...
def _mock_generic(qt_data: dict) -> dict:
    ...
    return {
        ...
        "book_overview": {
            "author": f"{book}의 저자는 여러 견해가 있어 단정하기 어려움",
            "date": {
                "covered": None,
                "written": f"{book}이 다루는 시대 이후 후대 정리 추정",
            },
            "place": "당시 하나님의 백성을 위해 기록됨.",
            "core": f"{book}은 하나님과 그 백성 사이의 관계, 그 안에서 일하시는 하나님의 손길을 보여주는 책.",
        },
...
def validate(ai_data: dict) -> list:
    ...
    # book_overview 검증: 4필드 객체이며 date는 객체. 옛 문자열도 폴백으로 허용.
    ...
    if not bo:
        ...
        # date 객체 검증
        date_val = bo.get("date")
        if isinstance(date_val, dict):
            if not date_val.get("written"):
                warnings.append("book_overview.date.written 비어 있음 (필수)")
        elif isinstance(date_val, str):
            warnings.append("book_overview.date가 문자열 형식 (객체 권장)")
