...
WEAK_SOURCES = {"본문 관찰 (GPT-4o 요약)", "개역개정 본문 관찰", "일반 지식(미확정)"}
...
4. source에는 근거를 **구체적으로** 적으세요.
   ...
   - 어원은 "Strong's H####"로, 자료 없이 본문 자체 관찰이면 "본문 관찰 (GPT-4o 요약)"로 적습니다(외부 근거 없이 본문만 읽고 정리했다는 뜻).
   - 근거가 불확실하면 "일반 지식(미확정)"이라고 정직히 적고, "공개 주석"처럼 뭉뚱그리지 마세요.
5. 과도한 도덕적 정죄나 인물 미화를 피하고, 본문이 말하는 범위를 넘지 마세요.
6. **"사본번역"은 주석에 그 언급이 실제로 있을 때만 적습니다.**
   개역개정의 어떤 낱말이 다른 사본·역본(마소라 본문·칠십인역·사해사본·시리아역 등)과 갈리는 곳입니다.
   사무엘상하는 마소라 본문의 손상이 많은 책이라 이런 대목이 실제로 나옵니다 — 있으면 놓치지 마세요.
   - ✓ "삼하 15:7 — 마소라 본문은 '사십 년', 여러 고대 역본은 '사 년'. 개역개정은 '사 년'을 따랐다."
   - ✗ **"성경에서 종종 ~하게 번역된다" 같은 일반화 금지.** 그 구절 하나의 문제인지 성경 전반의 현상인지
     구분해서 적으세요. 실제로 이 일반화가 거짓으로 나간 적이 있습니다(2026-08-04, 삼하 15:7).
   - 주석에 언급이 없으면 **이 갈래는 비웁니다.** 원문을 짐작해서 채우지 마세요.
   - 한국어 번역본(새번역·현대인의 성경 등) 비교는 자료가 없으므로 적지 마세요. 사본·고대 역본까지만.

7. **"문화관습"으로 태그를 붙이려면 그 장 안에서 딱 한 번 있었던 개인의 행동만으로 "당시 ~문화·관습을 보여준다"고 일반화하지 마세요.**
   ...
   — source가 "본문 관찰 (GPT-4o 요약)"이나 "일반 지식(미확정)"뿐이면 그건 아직 문화관습이라고 부를 근거가 부족하다는 뜻입니다.

[출력 형식 — 오직 JSON 객체 하나. 설명 문장 금지]
{
  "key_details": [
    {"verse": "장:절 또는 장:절-절", "cat": "지리|어원|문화관습|신학핵심|본문관찰|인물배경|사본번역",
  ...
  "신학_핵심": "이 장의 핵심 신학을 4~6문장으로 — 하나님의 성품·행하심, 언약/구속사적 위치, (본문·주석이 뒷받침하면) 신약에서의 성취까지. **끝에 근거를 괄호로 밝히세요** — 예: '(근거: [Gill · 삼하 7:16], [Keil-Delitzsch · 삼하 7:12])'. 주석이 뒷받침하면 그 꼬리표들을, 본문 관찰에서 종합한 것이면 '(근거: 본문 관찰·GPT-4o 정리)'라고 정직하게 적으세요.",
  "주의점": ["오해·과장을 막을 경고 + 주석마다 견해가 갈리는 지점을 구체적으로 (2~4개). **각 항목 끝에 근거를 괄호로 밝히세요** — 예: '...본다. (근거: [Keil-Delitzsch · 삼하 7:14])' 또는 견해가 갈리면 '(근거: Gill은 A로, K&D는 B로 본다)'. 외부 근거 없이 본문만 읽고 경고하는 것이면 '(근거: 본문 관찰·GPT-4o 정리)'."]
}
...
**신학_핵심과 주의점은 '해석'이므로, 각 진술 끝에 근거를 괄호로 밝히세요.** 이 코너는 나중에 사람이 신학적으로 검증하는데, 근거가 보여야 "이 해석이 어디서 왔는지"를 빠르게 확인할 수 있습니다. 공개 주석이 뒷받침하면 그 대괄호 꼬리표([Gill · 삼하 7:16])를 괄호 안에 옮겨 적고, 주석마다 견해가 갈리면 누가 어떻게 보는지 밝히세요(예: "Gill은 문자적 혈통으로, K&D는 메시아적으로 본다"). 외부 근거 없이 본문 관찰에서 종합한 것이면 정직하게 "(근거: 본문 관찰·GPT-4o 정리)"라고 적으세요 — 없는 주석가·구절을 지어내지 마세요.
...
def generate_chapter_kb(client, qt_data: dict, chapter: str, passage_nums=None) -> tuple[dict, dict]:
    ...
    # 주석(HelloAO 퍼블릭 도메인 6종) — 그날 본문의 '모든 절'에 붙인다.
    # 예전엔 biblehub를 절당 1요청으로 긁어와 무거워서 급소 3~8절만 붙였고, 그 바람에 KB
    # 절반이 외부 근거 없는 '본문 관찰 (GPT-4o 요약)'이 됐다(2026-07-21). 이제 로컬 DB(또는
    # 클라우드는 HelloAO 즉석 캐시)에서 읽으므로 전 절에 붙여도 비용·차단이 없다.
    commentary_text = None
    try:
        vc = commentary_source.chapter_commentary(book, int(chapter), verses=focus_nums, log=log)
        commentary_text = biblehub_lookup.format_commentary_for_prompt(vc, book, int(chapter))
        if commentary_text:
            log(f"  주석(PD 6종): {len(vc)}개 절 첨부(그날 본문 전 절)", "INFO")
    except Exception as e:
