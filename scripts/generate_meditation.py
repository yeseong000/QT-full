...
# 1 = 판 고르기 끄고 관문만 (2026-08-04 사장님 결정). 3판을 돌려 '덜 겹치는 판'을 고르는 건
# 임베딩 관문이 생기기 전의 처방이었다 — 지금은 관문이 겹치는 질문을 직접 빼내므로 같은 일을
# 두 번 하는 셈이다. 실비 실측(8/01 재생성): 3판 = 입력 252,221토큰 · 844원 vs 운영 128원.
# 인수인계 0803에도 "조기 종료를 1로 풀면 6판으로 줄고 최종 결과는 세 날 모두 동일"이란 기록이 있다.
FOLLOW_UP_BEST_OF_N = 1               # N번 뽑아 답변 겹침 최소 세트 자동 선택(1이면 사실상 단발)
